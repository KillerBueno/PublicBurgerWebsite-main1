...
function Screen1() {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', flex: 1, display: 'grid', gridTemplateColumns: '1fr 1px 1fr', gridAutoRows: '1fr', padding: '0 72px', overflow: 'hidden' }}>
        {col1.map((b, i) => (
          <div key={b.name} style={{ gridColumn: 1, gridRow: i + 1, padding: '0 40px 0 0', borderBottom: i < col1.length - 1 ? '1px solid rgba(26,10,16,0.07)' : 'none' }}>
            <BurgerItem burger={b} last />
          </div>
        ))}
        <div style={{ gridColumn: 2, gridRow: `1 / ${col1.length + 1}`, background: 'rgba(26,10,16,0.07)' }} />
        {col2.map((b, i) => (
          <div key={b.name} style={{ gridColumn: 3, gridRow: i + 1, padding: '0 40px', borderBottom: i < col2.length - 1 ? '1px solid rgba(26,10,16,0.07)' : 'none' }}>
            <BurgerItem burger={b} last />
          </div>
        ))}
      </div>
